...
def extract_json_from_llm_response(response):
    ...
    print(f"Extracting JSON from response (first 100 chars): {res_str[:100]}...")
    ...
    try:
        ...
        if think_idx != -1:
            ...
            print(f"Found </think> marker, extracting text after it")
            
        # Find JSON in the response
        idx = res_str.find("json:")
        if idx == -1:
            print(f"No 'json:' marker found in response")
            
            # Check if the response is already a valid JSON string
            try:
                clean_json = res_str.strip()
                if clean_json.startswith('{') and clean_json.endswith('}'):
                    print(f"Response appears to be a JSON object, attempting to parse directly")
        ...
        print(f"Found 'json:' marker at position {idx}")
            
        # Extract JSON part starting after 'json:'
        json_part = res_str[idx + 5:].strip()
        
        # Find the first '{' character
        start_idx = json_part.find('{')
        if start_idx == -1:
            print(f"No opening brace found after 'json:' marker")
            return None
            
        json_part = json_part[start_idx:]
        print(f"JSON part starts with: {json_part[:50]}...")
        ...
        print(f"Extracted JSON string (first 50 chars): {json_str[:50]}...")
        ...
        print(f"Successfully parsed JSON with {len(parsed_json.keys())} keys")
        return parsed_json
        
    except Exception as e:
        print(f"Error extracting JSON: {str(e)}")
        print(f"Error occurred at position: {e.__traceback__.tb_lineno}")
        return None
...
"""
    ...
    # Directly use the load balancer URL instead of environment variable
    lb_url = "http://balancer-1855376680.us-east-1.elb.amazonaws.com"
    ...
    while retry_count <= max_retries:
        try:
            ...
            print(f"Request URL: {lb_url}")
            print(f"Request Headers: {headers}")
            print(f"Request Payload (first 200 chars): {str(payload)[:200]}...")
            ...
            if response.status_code == 200:
                ...
                try:
                    ...
                    if 'choices' in response_json and len(response_json['choices']) > 0:
                        response_text = response_json['choices'][0].get('message', {}).get('content', '')
                        
                        # If we have content with a "json:" marker, extract the JSON part
                        if response_text and 'json:' in response_text:
                            print(f"Found 'json:' marker in the response")
                            return {"fullResponse": response_text}
                        elif response_json['choices'][0].get('text', ''):
                            # Fallback to 'text' field if 'message.content' is not available
                            response_text = response_json['choices'][0].get('text', '')
                            if 'json:' in response_text:
                                print(f"Found 'json:' marker in the 'text' field")
                                return {"fullResponse": response_text}
                    
                    # Try other formats if 'choices' doesn't work
                    for key in ['generation', 'response', 'content', 'output']:
                        if key in response_json:
                            response_text = response_json[key]
                            if response_text and isinstance(response_text, str) and 'json:' in response_text:
                                print(f"Found 'json:' marker in the '{key}' field")
                                return {"fullResponse": response_text}
                    
                    # If no usable content was found but we have a JSON response, return it as is
                    print(f"No 'json:' marker found, returning full response")
                    return {"fullResponse": json.dumps(response_json)}
                    
                except json.JSONDecodeError:
                    # If the response is not JSON, check for 'json:' directly in the text
                    if 'json:' in response.text:
                        print(f"Found 'json:' marker in non-JSON response")
                        return {"fullResponse": response.text}
                    else:
                        print(f"No usable response format found")
                        return {"fullResponse": response.text}
                ...
                if "Too many tokens" in response.text:
                    retry_count += 1
